...
async def get_campus_schedule(
    ...
    try:
        if featured_only or (not start_date and not end_date):
            ...
            sessions = CampusService.get_sessions(
                db=db,
                skip=skip,
                limit=limit,
    ...
    try:
        ...
        if current_user:
            ...
            if not booking.participant_email:
                booking.participant_email = current_user.email
            if not booking.participant_name:
                booking.participant_name = (
                    f"{current_user.first_name} {current_user.last_name}".strip()
                )
            if not booking.participant_phone and current_user.phone:
                booking.participant_phone = current_user.phone
...
async def get_my_bookings(
    ...
    bookings = CampusService.get_user_bookings(db, current_user.id)
    return bookings
